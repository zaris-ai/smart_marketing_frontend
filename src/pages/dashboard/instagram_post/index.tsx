...
const initialForm: InstagramPostForm = {
    ...
    visual_style: 'clean SaaS dashboard visuals, premium tech style, high-readability feed design',
    language: 'English',
    number_of_ideas: 5,
...
const InstagramPostIdeasPage = () => {
    ...
    const loadRuns = async () => {
        try {
            ...
            setError(
                ...
                err?.message ||
                'Failed to load previous Instagram Post runs.'
            );
        } finally {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const data: InstagramPostRun = response?.data?.data;

            if (!data || !Array.isArray(data.ideas)) {
                throw new Error('Invalid response from Instagram Post agent.');
            }
            ...
            setError(
                ...
                err?.response?.data?.message ||
                err?.message ||
                'Failed to generate Instagram Post ideas.'
            );
        } finally {
            setGenerating(false);
        }
    ...
    const handleDeleteRun = async () => {
        ...
        try {
            ...
            await api.delete(`/instagram-post-agent/post-ideas/${selectedRun._id}`);
            ...
            setError(
                ...
                err?.response?.data?.message ||
                err?.message ||
                'Failed to delete Instagram Post run.'
            );
        } finally {
            setDeleting(false);
        }
                        ...
                        >
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(event) => updateField('post_format', event.target.value)}
                                >
                                ...
                                <div className="space-y-3">
                                    {runs.map((run) => (
                                        <button
                                            ...
                                            className={`block w-full rounded-xl border p-4 text-left transition hover:border-blue-300 hover:shadow-sm ${selectedRun?._id === run._id
                    ...
                    <div className="space-y-6">
                        {generating ? (
                            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                                <p className="text-sm text-gray-500 dark:text-gray-400">
                                    Generating and saving Instagram Post ideas...
                                </p>
                            </div>
                        ) : !selectedRun ? (
                            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                                    ...
                                    <div className="grid gap-3 md:grid-cols-2">
                                        {selectedRun.ideas.map((idea, index) => (
                                            <button
                                                ...
                                                className={`rounded-xl border p-4 text-left transition hover:border-blue-300 hover:shadow-sm ${selectedIdeaIndex === index
                                                        ? 'border-blue-400 bg-blue-50 dark:border-blue-700 dark:bg-blue-950/30'
                                                        : 'border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900'
                                                    }`}
                                            >
                                    ...
                                    <div className="space-y-6">
                                        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                                            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                                                ...
                                                <Button
                                                    type="button"
                                                    onClick={() => copyToClipboard(selectedIdea.creative_prompt)}
                                                >
