...
function ProblemDiscoveryPage() {
  ...
  const parsedUrls = useMemo(
    () => normalizeUrlsTextToArray(form.urlsText),
    [form.urlsText]
  );
  ...
  const fetchRunById = async (id: string) => {
    try {
      ...
      setServerError(getErrorMessage(error, 'Failed to fetch run details.'));
    } finally {
      setIsLoadingDetails(false);
    }
  };

  const fetchRuns = async (preferredId?: string) => {
    try {
      ...
      if (preferredId) {
        const matched = items.find((item: ProblemDiscoveryRunDoc) => item._id === preferredId);

        if (matched?._id) {
          await fetchRunById(matched._id);
          return;
        }
      }
      ...
      if (
        selectedRun &&
        !items.some((item: ProblemDiscoveryRunDoc) => item._id === selectedRun._id)
      ) {
        if (items[0]?._id) {
          await fetchRunById(items[0]._id);
        } else {
      ...
      setServerError(
        getErrorMessage(error, 'Failed to fetch problem discovery runs.')
      );
    } finally {
    ...
    const timer = setInterval(async () => {
      try {
        ...
        if (run.status === 'success') {
          ...
          await fetchRuns(savedId);

          toast.success('Problem discovery completed and saved.');
        ...
        if (run.status === 'failed') {
          ...
          setServerError(run?.error?.message || 'Problem discovery task failed.');
          toast.error(run?.error?.message || 'Problem discovery task failed.');
        ...
        if (run.status === 'cancelled') {
          ...
          setServerError(run?.error?.message || 'Problem discovery task was cancelled.');
          toast.error('Problem discovery task was cancelled.');
        ...
        setServerError(
          getErrorMessage(
            error,
            'Problem discovery completed, but the result could not be loaded.'
          )
        );
  ...
  const onRunCrew = async () => {
    try {
      ...
      if (nextRunId) {
        ...
        toast.success(response?.data?.message || 'Problem discovery started in background.');
        return;
      }
      ...
      if (doc?._id) {
        await fetchRuns(doc._id);
        await fetchRunById(doc._id);
      }
    } catch (error: any) {
      setServerError(getErrorMessage(error, 'Failed to run problem discovery.'));
      toast.error(getErrorMessage(error, 'Failed to run problem discovery.'));
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const onCancelRun = async () => {
    ...
    try {
      ...
      setIsBackgroundRunning(false);
      setIsSubmitting(false);
      setServerError('Problem discovery task was cancelled.');
      toast.error('Problem discovery task was cancelled.');

      setRunId('');
    } catch (error: any) {
      setServerError(getErrorMessage(error, 'Failed to stop problem discovery task.'));
      toast.error(getErrorMessage(error, 'Failed to stop problem discovery task.'));
    }
  };
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      toast.success('Problem discovery run deleted.');
    } catch (error: any) {
      setServerError(
        getErrorMessage(error, 'Failed to delete problem discovery run.')
      );
      toast.error(getErrorMessage(error, 'Failed to delete problem discovery run.'));
    } finally {
      setIsDeleting(false);
    }
  };
            ...
            <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm text-gray-500 dark:text-gray-400">
                  Review submitted links, extract user problems and questions from those pages,
                  answer them, and assess whether Arka can solve them now or what feature gaps should be added.
                </p>
                ...
                {isBackgroundRunning && (
                  <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700 dark:border-blue-900/40 dark:bg-blue-950/40 dark:text-blue-300">
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        Problem discovery is running in background. Status:{' '}
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Button
                  ...
                  disabled={isBusy}
                >
                  {isBackgroundRunning ? 'Running in Background...' : 'Run Discovery'}
                </Button>
...
https://www.reddit.com/r/shopify/...`}
                      className="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-900 disabled:opacity-60 dark:border-gray-700 dark:bg-gray-950 dark:text-white dark:focus:border-white"
                    />
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      Review the selected run, submitted sources, and app reference used for fit analysis.
                    </p>
                  </div>
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      Each item includes the question, answer, exact source page, and Arka fit analysis.
                    </p>
                  </div>
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          <div className="grid gap-4 md:grid-cols-2">
                            ...
                            <div className="rounded-2xl border border-gray-200 p-4 dark:border-gray-800">
                              ...
                              <p className="mt-2 text-sm leading-6 text-gray-700 dark:text-gray-300">
                                {item.arka_solution ||
                                  'No direct current solution identified.'}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="grid gap-4 md:grid-cols-2">
                            ...
                            <div className="rounded-2xl border border-gray-200 p-4 dark:border-gray-800">
                              ...
                              <p className="mt-2 text-sm leading-6 text-gray-700 dark:text-gray-300">
                                {item.recommended_feature ||
                                  'No additional feature recommendation.'}
                              </p>
                            </div>
                          </div>
