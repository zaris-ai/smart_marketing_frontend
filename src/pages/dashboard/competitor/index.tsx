...
const CompetitorAnalysisPage = () => {
  ...
  const [latestDoc, setLatestDoc] = useState<CompetitorAnalysisDoc | null>(null);
    ...
    try {
      ...
      setServerError(
        error?.response?.data?.message ||
          ...
          'Failed to fetch latest competitor analysis.'
      );
    } finally {
    ...
    try {
      ...
      setServerError(
        error?.response?.data?.message ||
          error?.response?.data?.detail ||
          error?.response?.data?.error ||
          'Failed to run competitor analysis crew.'
      );
    } finally {
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                  View the latest saved analysis first, then generate and save a new one.
                </p>
